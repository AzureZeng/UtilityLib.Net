...
public static class Json2Resource {
    ...
    public static void Convert(ReadOnlySpan<byte> input, Stream target, string fileItemPrefix = "$file:",
        string contextPath = "") {
        using var resw = new ResourceWriter(target);
    ...
    private static void Json2ResourceInternal(ResourceWriter resw, ref JsonElement element, string ns,
        string fileItemPrefix, string contextPath, List<Stream> streamList) {
        ...
        foreach (var p in element.EnumerateObject()) {
            ...
            switch (p.Value.ValueKind) {
                ...
                case JsonValueKind.Object:
                    var sub = p.Value;
                    Json2ResourceInternal(resw, ref sub, $"{finalNs}{p.Name}", fileItemPrefix, contextPath, streamList);
                    break;
                default:
